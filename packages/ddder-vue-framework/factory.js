...
function updateListeners (
  ...
  if (toAdd.length) {
    ...
    for (var i = 0; i < toAdd.length; i++) {
      var event$1 = toAdd[i];
      add(event$1.name, event$1.handler, event$1.once, event$1.capture, event$1.passive, event$1.addition);
    }
  }
  for (name in oldOn) {
    if (isUndef(on[name])) {
...
function updateInstanceXTemplates (oldVNode, vnode) {
  ...
  var nodeXTemplateMaps = data.xTemplateMaps;
  var instanceXTemplateMaps = context._xTemplateMaps;
...
var patchSequence = [textStyle];

function patchStylesheet(elm, stylesheet) {
  patchSequence.forEach(function (middleware) {
...
var isReservedTag$1 = makeMap(makeReservedTemplateTags());
...
var xTemplate$1 = {
  endTransformNode: endTransformNode,
  genData: genData$1
};
...
Vue$2.prototype.$mount = function (el, hydrating) {
  ...
  if (!options.render) {
    ...
    if (template) {
      var ref = compileToFunctions(template, { delimiters: options.delimiters }, this);
      var render = ref.render;
      var staticRenderFns = ref.staticRenderFns;
